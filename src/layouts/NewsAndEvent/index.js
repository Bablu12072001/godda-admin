...
//         "https://kxu5bktpoi.execute-api.ap-south-1.amazonaws.com/JMOA/jmoa_blog_edit",
//         {
//           id: editid,
//           title: editTitle,
//           author: editAuthor,
...
//           prevData.map((data) =>
//             data.id === editid
//               ? { ...data, title: editTitle, author: editAuthor, content: editContent }
//               : data
//           )
//         );
...
//         "https://kxu5bktpoi.execute-api.ap-south-1.amazonaws.com/JMOA/jmoa_news_events_delete",
//         {
...
//       if (response.data["body-json"].statusCode === 200) {
//         setNewsData((prevData) => prevData.filter((Data) => Data.id !== selectedItem?.id));
...
//           "https://kxu5bktpoi.execute-api.ap-south-1.amazonaws.com/JMOA/jmoa_news_events_all_data"
//         );
...
//                   <TableCell>
//                     {item.imageUrl.length > 0 && <Avatar alt="Image" src={item.imageUrl[0]} />}
//                   </TableCell>
//                   <TableCell>{truncateString(item.title, 3)}</TableCell>
//                   <TableCell>
...
const NewsEventTable = () => {
  ...
  const [selectedItemDeletedata, setSelectedItemDeletedata] = useState(null);
  const [openConfirmation, setOpenConfirmation] = useState(false);
  const [loading, setLoading] = useState(false); // State for API call loading indicator
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        "https://vkfpe87plb.execute-api.ap-south-1.amazonaws.com/production/jmoa_news_events_edit",
        {
          id: editid,
          title: editTitle,
      ...
      console.log("edit data is ", response.data);

      if (response.data["body-json"].statusCode === 200) {
        // Update the data in the state
        setNewsData((prevData) => prevData.map((data) => (data.id === editid ? { ...data, title: editTitle, content: editContent } : data)));
        console.log("Blog edited successfully!");
        setEditDialogOpen(false);
      } else {
        console.log("Failed to edit blog.");
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      setLoading(true); // Set loading state to true before API call
      const response = await axios.delete("https://vkfpe87plb.execute-api.ap-south-1.amazonaws.com/production/jmoa_news_events_delete", {
        data: { id: selectedItemDeletedata },
        headers: {
          Authorization: accessToken(),
        },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true); // Set loading state to true before API call
        const response = await axios.get("https://vkfpe87plb.execute-api.ap-south-1.amazonaws.com/production/jmoa_news_events_all_data");
        setNewsData(response.data["body-json"].body);
